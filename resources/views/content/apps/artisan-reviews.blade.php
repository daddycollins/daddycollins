        ...
        <table class="table table-hover mb-0">
          ...
          <tbody>
            @forelse($reviews as $review)
              @php
                $reviewType = 'all';
                if ($review->rating >= 4 && !$review->has_response) {
                    $reviewType = 'positive-no-response';
                } elseif ($review->rating >= 4) {
                    $reviewType = 'positive';
                }
                if (!$review->has_response) {
                    $reviewType = 'no-response';
                }
              @endphp
              <tr data-review-type="{{ $reviewType }}">
                <td>
                ...
                <td>
                  <small>"{{ Illuminate\Support\Str::limit($review->comment, 50) }}"</small>
                </td>
...
@section('page-script')
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      // Review Filter by Type
      const filterInputs = document.querySelectorAll('input[name="reviewFilter"]');
      const tableRows = document.querySelectorAll('table tbody tr[data-review-type]');
      ...
      filterInputs.forEach(input => {
        input.addEventListener('change', function(e) {
          ...
          tableRows.forEach(row => {
            const rowType = row.getAttribute('data-review-type');

            if (selectedFilter === 'all') {
              row.style.display = '';
            } else if (selectedFilter === 'positive') {
              // Show reviews with rating >= 4
              if (rowType === 'positive' || rowType === 'positive-no-response') {
                row.style.display = '';
              } else {
                row.style.display = 'none';
              }
            } else if (selectedFilter === 'no-response') {
              // Show reviews with no response
              if (rowType === 'no-response' || rowType === 'positive-no-response') {
                row.style.display = '';
              } else {
                row.style.display = 'none';
              }
          ...
          sessionStorage.setItem('reviewFilter', selectedFilter);
        });
      });

      // Restore filter preference from sessionStorage
      const savedFilter = sessionStorage.getItem('reviewFilter') || 'all';
      const filterRadio = document.querySelector(`input[name="reviewFilter"][value="${savedFilter}"]`);
      if (filterRadio) {
        filterRadio.click();
      }
