...
@section('content')
        ...
        <div class="card-body">
          <div class="d-flex align-items-center justify-content-between">
            <div>
              ...
              <p class="mb-0 text-success"><i class="icon-base ri ri-arrow-up-s-line"></i> +{{ $reviewsThisMonth }} this month</p>
            </div>
        ...
        <div class="card-body">
          <div class="d-flex align-items-center justify-content-between">
            <div>
              ...
              <p class="mb-0 {{ $averageRating >= 4 ? 'text-success' : ($averageRating >= 3 ? 'text-warning' : 'text-danger') }}">
          ...
          <div class="review-list">
            @forelse($reviews as $review)
              @php
                ...
                $ratingBadgeClass = $rating >= 5 ? 'bg-label-success' : ($rating >= 4 ? 'bg-label-info' : ($rating >= 3 ? 'bg-label-warning' : 'bg-label-danger'));
              @endphp
              <!-- Dynamic Review Item -->
              <div class="review-item {{ !$loop->last ? 'border-bottom pb-4 mb-4' : '' }}">
                <div class="d-flex justify-content-between align-items-start mb-3">
                  <div class="d-flex align-items-start gap-3 flex-grow-1">
                    <img src="{{ $artisan->profile_photo_path ? asset('storage/' . $artisan->profile_photo_path) : asset('assets/img/avatars/1.png') }}"
                      ...
                      @if($review->has_response && $review->response_comment)
                        <div class="bg-light p-3 rounded mb-3">
                          <div class="d-flex align-items-center gap-2 mb-2">
                            ...
                            <small class="text-muted">{{ $review->response_date ? \Carbon\Carbon::parse($review->response_date)->format('M d, Y') : '' }}</small>
                          </div>
            ...
            @empty
              ...
              <div class="text-center py-5">
                ...
                <p class="text-muted mb-4">You haven't submitted any reviews yet. Complete an order to leave a review!</p>
              ...
              <tbody>
                @foreach($pendingReviewOrders as $order)
                  ...
                  <tr>
                    ...
                    <td>
                      <button class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#createReviewModal"
  ...
  <div class="modal fade" id="createReviewModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        ...
        <form action="#" method="POST" id="createReviewForm">
          @csrf
          <div class="modal-body">
              ...
              <select class="form-select" name="order_id" id="orderSelect" required>
                ...
                @foreach($pendingReviewOrders as $order)
                  @php
                    ...
                    $serviceName = $order->items->first()?->artisanService?->service_name ?? 'Service';
                  @endphp
                  <option value="{{ $order->id }}" data-artisan="{{ $artisanName }}" data-category="{{ $artisan->category ?? 'General' }}">
              ...
              <div class="d-flex gap-3" id="ratingStars">
                @for($i = 1; $i <= 5; $i++)
                  <div class="rating-input">
                    <i class="icon-base ri ri-star-line icon-32px text-muted cursor-pointer" data-rating="{{ $i }}"></i>
                  </div>
                @endfor
              </div>
            ...
            <div class="mb-4">
              ...
              <textarea class="form-control" name="comment" rows="5" placeholder="Share your experience with this artisan..." required></textarea>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      ...
      const chartData = [{{ $fiveStarCount }}, {{ $fourStarCount }}, {{ $threeStarCount }}, {{ $twoStarCount }}, {{ $oneStarCount }}];
      ...
      if (hasData) {
        ...
        document.querySelector("#ratingDistributionChart").innerHTML = '<div class="text-center py-5 text-muted"><i class="ri-pie-chart-line" style="font-size: 48px;"></i><p class="mt-2">No review data yet</p></div>';
      }
