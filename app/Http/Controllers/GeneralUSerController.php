...
class GeneralUSerController extends Controller
{
    ...
    public function showArtisanProfile(ArtisanProfile $artisan)
    {
        ...
        // Only show verified artisans
        if (!$artisan->verified) {
            return redirect()->route('user-browse-artisans')
                ->with('error', 'This artisan profile is not available');
        }

        // Calculate statistics
        $stats = [
            'totalReviews' => $artisan->reviews->count(),
            'averageRating' => $artisan->average_rating ?? 0,
            'completedOrders' => $artisan->orders()->where('status', 'completed')->count(),
            'totalServices' => $artisan->services->count(),
            'totalProducts' => $artisan->goods->count(),
            'responseRate' => $this->calculateResponseRate($artisan),
            'completionRate' => $this->calculateCompletionRate($artisan),
        ];

        // Rating breakdown
        $ratingBreakdown = [];
        for ($i = 5; $i >= 1; $i--) {
            $count = $artisan->reviews->where('rating', $i)->count();
            $ratingBreakdown[$i] = [
                'count' => $count,
                'percentage' => $stats['totalReviews'] > 0
                    ? round(($count / $stats['totalReviews']) * 100)
                    : 0
            ];
        }

        return view('content.apps.artisan-public-profile', compact(
            'artisan',
            'stats',
            'ratingBreakdown'
        ));
    ...
    private function calculateResponseRate(ArtisanProfile $artisan): int
    {
        $totalReviews = $artisan->reviews()->count();
        if ($totalReviews === 0) {
            return 100;
        ...
        $responded = $artisan->reviews()->where('has_response', true)->count();
        return round(($responded / $totalReviews) * 100);
    }

    private function calculateCompletionRate(ArtisanProfile $artisan): int
    {
        $totalOrders = $artisan->orders()->count();
        if ($totalOrders === 0) {
            return 100;
        }

        $completed = $artisan->orders()->where('status', 'completed')->count();
        return round(($completed / $totalOrders) * 100);
    }
        ...
        if (request()->filled('search')) {
            ...
            $query->where(function ($q) use ($search) {
                $q->where('id', 'like', '%' . $search . '%')
                    ->orWhereHas('artisan.user', function ($subQ) use ($search) {
    ...
    public function trackOrder(Order $order)
    {
        ...
        return view('content.apps.order-tracking', compact('order'));
    }
    ...
    public function downloadInvoice(Order $order)
    {
        ...
        // For now, redirect to order details with success message
        return redirect()->route('user-order-details', $order->id)->with('success', 'Invoice feature coming soon');
    }

    public function createReviewForOrder(Order $order)
    {
        // Check authorization
        if ($order->client_id !== Auth::id()) {
            return redirect()->route('user-my-orders')->with('error', 'Unauthorized access');
        ...
        if ($order->review()->exists()) {
            return redirect()->route('user-create-review')->with('info', 'You have already reviewed this order');
        }

        return redirect()->route('user-create-review')->with('order_id', $order->id);
    }

    public function contactArtisan(Order $order)
    {
        // Check authorization
        if ($order->client_id !== Auth::id()) {
            return redirect()->route('user-my-orders')->with('error', 'Unauthorized access');
        ...
        $artisan = $order->artisan()->with('user')->first();
        if (!$artisan) {
            return redirect()->route('user-my-orders')->with('error', 'Artisan not found');
        }

        return view('content.apps.artisan-contact', compact('order', 'artisan'));
    }
    ...
    public function payNow(Order $order)
    {
        ...
        if ($order->payment_status !== 'unpaid') {
            return redirect()->route('user-my-orders')->with('error', 'This order is already paid');
        }

        return redirect()->route('user-order-details', $order->id)->with('info', 'Payment processing coming soon');
    }
    ...
    public function storeReview()
    {
        ...
        // Check if review already exists
        $existingReview = Review::where('order_id', $order->id)->where('client_id', $user->id)->first();
        if ($existingReview) {
        ...
        return redirect()->route('user-create-review')->with('success', 'Review submitted successfully');
    }
    ...
    public function updateReview(Review $review)
    {
        ...
        return redirect()->route('user-view-review', $review->id)->with('success', 'Review updated successfully');
    }
