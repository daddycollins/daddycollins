...
{
    /**
     * Toggle the AUTO_VERIFY_ARTISAN config value via AJAX.
     */
    public function toggleAutoVerify(Request $request)
    {
        // Optionally restrict to artisan role
        $request->validate([
        ...
        if (preg_match('/^AUTO_VERIFY_ARTISAN=/m', $envContent)) {
            $envContent = preg_replace('/^AUTO_VERIFY_ARTISAN=.*/m', 'AUTO_VERIFY_ARTISAN=' . ($enabled ? 'true' : 'false'), $envContent);
        } else {
            $envContent .= "\nAUTO_VERIFY_ARTISAN=" . ($enabled ? 'true' : 'false');
        }
...
// Total and monthly orders
    ...
    public function verification()
    {
        ...
        return view('content.apps.artisan-verification', compact(
            'user',
            'artisanProfile',
            'nationalDocument',
            'verification',
...
// For now, return success response with order details
    ...
    public function updatePersonalDetails(Request $request)
    {
        ...
        $eloquentUser->name = $validated['name'];
        $eloquentUser->email = $validated['email'];
        $eloquentUser->save();
    ...
    public function changePassword(Request $request)
    {
        ...
        $eloquentUser = \App\Models\User::find($user->id);
        $eloquentUser->password = bcrypt($validated['new_password']);
        $eloquentUser->save();
